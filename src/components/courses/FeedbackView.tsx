...
import { submitCourseFeedback, getCourseFeedback } from '../../services/mockData';
...
const FeedbackView: React.FC<FeedbackViewProps> = ({ courseId, userRole }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // In a real app, we would get the actual student ID from auth context
      const mockStudentId = '2569004'; 
      await submitCourseFeedback(courseId, mockStudentId, rating, comment);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Course Feedback Summary</h2>
          <p className="text-sm text-gray-500 mt-1">Based on {feedbacks.length} student responses</p>
        </div>
      ...
      <div className="space-y-4">
        {feedbacks.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <p className="text-gray-500 mt-2">Students haven't submitted any feedback for this course yet.</p>
          </div>
        ) : (
          feedbacks.map((fb) => (
            <div key={fb.id} className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">Student ID: {fb.studentId}</p>
                ...
                <div className="flex items-center bg-yellow-50 px-2 py-1 rounded">
                  <span className="text-sm font-bold text-yellow-700 mr-1">{fb.rating}</span>
                  <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                </div>
              </div>
              <div className="mt-3 pl-11">
                <p className="text-gray-700 text-sm leading-relaxed">{fb.comments}</p>
              </div>
            </div>
          ))
        )}
